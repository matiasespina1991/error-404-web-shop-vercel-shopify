...
export async function getCollectionProducts({
  ...
  if (!endpoint) {
    ...
    if (collection === "hidden-homepage-carousel") {
      return MOCK_CAROUSEL_PRODUCTS;
    }

    return getMockProducts({ collection, reverse, sortKey });
  }
...
export async function getMenu(handle: string): Promise<Menu[]> {
  ...
  if (!endpoint) {
    return MOCK_COLLECTIONS.slice(1).map((collection) => ({
...
export async function getProductRecommendations(
  ...
  if (!endpoint) {
    return MOCK_PRODUCTS.filter((product) => product.id !== productId).slice(
      0,
      4,
    );
  }
...
export async function getProducts({
  ...
  if (!endpoint) {
    return getMockProducts({ query, reverse, sortKey });
  }
